...
def insert_into_bigquery(response):
    ...
    enriquecimiento_output = str(enriquecimiento_dict) #str(response["Atributos"])
    match_dict = {item['Atributo']: item['Predicciones'][0]['Match'] for item in atributos}
    match_output = str(match_dict) #str(response["Atributos"][0]["Predicciones"][0]["Match"])
    fecha = response["date"]
...
def generate_images(request: ImageRequest) -> list:
    ...
    print("enriquecimeinto:", enriquecimiento_response.text)
    # Comparacion
    ...
    print("comparacion:", response_comparacion.text)
