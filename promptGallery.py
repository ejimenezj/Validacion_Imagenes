...
def setPrincipalDetalle(plantilla, medidas, atributos):
    ...
    - Para  las imágenes principales y de detalle, plasma los siguientes atributos  en un formato json, "Atemporal", 
    ...
    si lo tiene 
    - En Sin referencias pon "True" si no tiene alguna referencia a marcas que son ajenas a Liverpool 
    y si no lo tiene pon "False" 
    -En Sin cruce de marcas pon "True" si no aparece mas de una marca en la imagen 
    ...
    muy oscura o muy luminosa, si no es así ponme "False" 
    - En Sin reflejos pon "True" si la imagen no se ve 
...
def Enriquecimiento_imagenes(atributos_descripcion):
    return f"""
    Eres un experto en administrar categorías e información de productos para el catálogo de productos de 
    una tienda departamental. 
    Tu misión es la siguiente: se te presentarán diferentes tipos de imágenes y con
    base en lo que observes en ellas y las instrucciones que se te proporcionen, deberás abordar los siguientes puntos.

    [CONSIDERACIONES] 
    - Evalúa cuidadosamente la información de cada una de las imágenes proporcionadas.
    - Toma tu tiempo para entender las funcionalidades y características del producto.
    - En 'ATRIBUTOS CON SU DESCRIPCIÓN' se incluye un diccionario con los atributos y su descripción.
    - En caso de que un atributo tenga descripción, ésta sirve como información adicional para generar su valor predicho.

    [INSTRUCCIONES] 
    - Devuelve un JSON completo en el siguiente formato: Atributo real(str):Valor predicho(str).
    - Si no se puede generar un valor predicho, escribir una cadena vacía.

    [ATRIBUTOS CON SU DESCRIPCIÓN]
    Atributos y su descripción: {atributos_descripcion}
    """

def comparacion(atributos_predichos, atributos_reales):
    return f"""
    [CONTEXTO]
    Eres un experto comparando información, datos, e imágenes e identificando las diferencias entre ellos.
    Tu misión es decirme si la información proporcionada es idéntica o no, es decir; si hacen match o no, dependiendo a las reglas que te indicare en instrucciones. 
    ...
    [CONSIDERACIONES]
    - Los atributos reales están en formato JSON con el siguiente formato: Atributo(str):Valor(str).
    - Los atributos predichos están en formato JSON con el siguiente formato: Atributo(str):Predicción(str).
    
    [INSTRUCCIONES]
    Debes de devolver en formato JSON en el formato Atributo (str):Match(bool) siguiendo el siguiente proceso:
    - Determina si los valores reales en 'atributos reales' y las predicciones en 'atributos predichos' son equivalentes, 
    sinónimos o comparten cierta superposición, en caso de ser así, coloca 'TRUE'; de lo contrario, coloca 'FALSE'.
